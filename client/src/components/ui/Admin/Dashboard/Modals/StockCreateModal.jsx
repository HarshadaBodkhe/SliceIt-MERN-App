...
function StockCreateModal({ onClose }) {
  ...
  return (
    <div
      className={`fixed inset-0 flex flex-col items-center justify-center z-10 bg-black/50 p-4 backdrop-filter backdrop-blur-sm transition-opacity duration-200 ${
        modalVisible ? 'opacity-100' : 'opacity-0 delay-150'
      }`}
      onClick={handleModalClose}
    >
      <div
        className="bg-white rounded-2xl p-6 w-full sm:w-2/3 md:w-1/2 shadow-2xl border border-gray-200"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="w-full text-xl text-primary border-b-2 border-secondary pb-2">
            Add a New Stock
          </h2>
          <button
            className="text-secondary border-2 border-secondary hover:bg-secondary transition-all duration-300 hover:text-white rounded-full p-1 ml-10"
            onClick={handleModalClose}
          >
            <FaTimes />
          </button>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center w-full">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  {stockTypes.map((type, index) => (
                    <label
                      htmlFor={type}
                      key={index}
                      className="flex items-center justify-center text-white rounded-full bg-primary hover:bg-secondary transition-all duration-300 px-4 py-2 cursor-pointer"
                    >
                      <input
                        type="radio"
                        name="item"
              ...
              <form
                ...
                className="w-full border border border-gray-200 bg-gray-50 rounded-2xl p-4"
              >
                ...
                <div className="w-full grid grid-cols-1 gap-4 md:grid-cols-2 my-2">
                  <div className="w-full">
                    ...
                    <input
                      ...
                      required
                      className="w-full text-primary bg-white placeholder-gray-400 border border-gray-200 focus:ring-2 focus:ring-primary focus:outline-none rounded-md p-4 pr-12 text-sm shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                  </div>
